...
#!/bin/bash
echo "[$0] Creating Java truststore...";
...
if [ ! -f "truststore.jks" ]; then
  ...
  openssl req -new -key fakecert.key -out fakecert.csr -subj "/C=CZ/ST=Czech Republic/L=Prague/O=BCV/CN=Cas management placeholder cert";
  ...
  echo "JAVA_OPTS=\"\$JAVA_OPTS -Djavax.net.ssl.trustStore=$truststorepath -Djavax.net.ssl.trustStorePassword=$storepass\"" > "$RUNSCRIPTS_PATH/startTomcat.d/001_005-TruststoreOpts.sh";
fi

echo "Found truststore $CASMANAGEMENT_CONFIG";

if [ ! -z "$(ls -A $CASMANAGEMENT_START/trustcert)" ]; then
  # trustcert directory is not empty, we import certificates
  storepass=$(cat truststore.pwfile);
  for crtfile in $CASMANAGEMENT_START/trustcert/*; do
  ...
  for crtalias in $(keytool -list -keystore truststore.jks -storepass "$storepass" | grep -v "^placeholder-cert" | awk -F, '$0~/trustedCertEntry/ {print $1}'); do
    ...
    if [ ! -f "$CASMANAGEMENT_START/trustcert/$crtalias" ]; then
